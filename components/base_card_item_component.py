...
class BaseCardItemComponent(BaseComponent):
    ...
    def check_visible(self, name: str, nth: int = 0):
        self.image.check_visible(nth)

        self.name.check_visible(nth)
        self.name.check_have_text(name, nth)

        self.price.check_visible(nth)
        self.total_price.check_visible(nth)


    @allure.step("Getting product price")
    def get_price(self, nth: int = 0) -> float:
        price = self.price.get_inner_text(nth)
        return float(price.replace("$", ""))

    @allure.step("Getting product total price")
    def get_total_price(self, nth: int = 0) -> float:
        total_price = self.total_price.get_inner_text(nth)
